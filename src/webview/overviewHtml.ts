...
function renderProjectGraphSection(data?: OverviewHtmlInput['projectGraphData']): string {
  ...
  const sortedProjects = [...projects].sort((a, b) => {
    const order: Record<string, number> = { frontend: 0, mobile: 1, backend: 2, shared: 3, database: 4, infra: 5 };
    return (order[(a as Record<string, unknown>)['kind'] as string] ?? 9) - (order[(b as Record<string, unknown>)['kind'] as string] ?? 9);
  });

  const linksByPair = new Map<string, { count: number; conf: string }>();
  ...
  const frontendProjects = sortedProjects.filter((p) => (p as Record<string, unknown>)['kind'] === 'frontend' || (p as Record<string, unknown>)['kind'] === 'mobile');
  const backendProjects = sortedProjects.filter((p) => (p as Record<string, unknown>)['kind'] === 'backend' || (p as Record<string, unknown>)['kind'] === 'shared');
  const databaseProjects = sortedProjects.filter((p) => (p as Record<string, unknown>)['kind'] === 'database');
  const infraProjects = sortedProjects.filter((p) => (p as Record<string, unknown>)['kind'] === 'infra');

  const renderLayer = (label: string, projs: unknown[], color: string) => {
    ...
    return `<div class="mpg-layer">
      ...
      <div class="mpg-layer-cards">
        ${projs.map((p) => {
          ...
          const stack = (proj['stack'] as string[] | undefined)?.slice(0, 3).join(', ') ?? '';
